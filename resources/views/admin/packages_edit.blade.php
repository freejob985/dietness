...
<!--begin::Card-->
										<div class="card card-custom gutter-b example example-compact">
											<div class="card-header">
												<h3 class="card-title">{{__('admin.packages.edit_package')}}</h3>
												
											</div>
											<!--begin::Form-->
											<form action="{{route($prefix.'_edit_post_packages',['id' => $package->id])}}" method="post" enctype="multipart/form-data">
                                                                                   {{csrf_field()}}
												<div class="card-body">
													<div class="form-group">
														<label>{{__('admin.packages.name_en')}}</label>
														<input type="text" class="form-control" name="name_en" value="{{$package->title_en}}">
													</div>
                                                    <div class="form-group">
                                                            <label>{{__('admin.packages.name_ar')}}</label>
                                                            <input type="text" class="form-control" name="name_ar" value="{{$package->title_ar}}">
                                                        </div>
											...
											<div class="dropzone_gallery" style="background-image:url({{asset('/uploads/packages/'.$package->image)}})">
