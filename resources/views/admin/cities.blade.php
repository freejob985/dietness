...
<div class="card card-custom">
							<div class="card-header flex-wrap border-0 pt-6 pb-0">
										<div class="card-title">
											<h3 class="card-label">{{__('admin.cities')}}</h3>
										</div>
										<div class="card-toolbar">
											<!--begin::Button-->
											<a href="{{route($prefix.'_cities_add_get')}}" class="btn btn-primary font-weight-bolder">
...
</svg><!--end::Svg Icon--></span>{{__('admin.Cities.new')}}</a>
											<!--end::Button-->
										</div>
									</div>		
									<div class="card-body">
										...
										<table class="datatable datatable-bordered datatable-head-custom" id="kt_datatable">
											<thead>
												<tr>
													<th title="Field #1">#</th>
													<th title="Field #2">{{__('admin.Cities.name')}}</th>
													<th title="Field #3">{{__('admin.Cities.governorate')}}</th>
													<th title="Field #4">{{__('admin.Cities.created_at')}}</th>
													<th title="Field #5">{{__('admin.Cities.options')}}</th>
											...
											<tbody>
												...
												<tr>
													...
													<td>{{$city->name}}</td>
													<td>{{$city->governorate_obj->name}}</td>
													<td>{{Carbon::parse($city->created_at)->format('Y-m-d')}}</td>
													<td>
                                                        <a title="{{__('admin.main_categories.edit_main_categories')}}" href="{{route($prefix.'_cities_edit_get',['id' => $city->id])}}"><i class="icon-xl la la-edit"></i></a>
                                                        <a title="{{__('admin.main_categories.remove_main_categories')}}" href="#" class="remove_btn" data="{{route($prefix.'_cities_delete',['id' => $city->id])}}"><i class="icon-xl la la-trash"></i></a>
                                                    </td>
