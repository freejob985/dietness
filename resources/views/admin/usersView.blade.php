...
<!--begin::Card-->
										<div class="card card-custom gutter-b example example-compact">
											<div class="card-header">
												<h3 class="card-title">{{__('admin.users.edit_user')}} - {{$user->name}}</h3>
												
											</div>
											<!--begin::Form-->
											<form action="{{route($prefix.'_post_edit_users',['user' => $user->id])}}" method="post">
                                                                                   {{csrf_field()}}
												<div class="card-body">
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <label>{{__('admin.users.country_code')}}</label>
                                                            <input type="text" class="form-control" name="country_code" value="{{$user->country_code}}">
                                                        </div>
                                                            <div class="col-lg-10">
                                                    ...
                                                    <div class="form-group ">
															...
															<select class="form-control m-select2" id="statuses" name="status">
																...
																<option value="{{$status}}" @if($status == $user->status) selected @endif>{{__('admin.users.statuses.'.$status)}}</option>
																@endforeach
															</select>
