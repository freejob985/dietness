...
<div class="card card-custom">
							<div class="card-header flex-wrap border-0 pt-6 pb-0">
										<div class="card-title">
											<h3 class="card-label">{{__('admin.drivers.driversTxt')}}</h3>
										</div>
										<div class="card-toolbar">
											<!--begin::Button-->
											<a href="{{route($prefix.'_add_drivers')}}" class="btn btn-primary font-weight-bolder">
...
</svg><!--end::Svg Icon--></span>{{__('admin.drivers.add_drivers')}}</a>
											<!--end::Button-->
										</div>
									</div>		
									<div class="card-body">
										...
										<table class="datatable datatable-bordered datatable-head-custom" id="kt_datatable">
											<thead>
												<tr>
													<th title="Field #1">#</th>
													<th title="Field #2">{{__('admin.drivers.name')}}</th>
													<th title="Field #3">{{__('admin.drivers.email')}}</th>
													<th title="Field #4">{{__('admin.drivers.mobile')}}</th>
													<th title="Field #5">{{__('admin.drivers.created_at')}}</th>
													<th title="Field #5">{{__('admin.drivers.updated_at')}}</th>
													<th title="Field #5">{{__('admin.drivers.options')}}</th>
											...
											<tbody>
                                             @foreach($getDrivers as $driver)
                                                <tr>
                                                 ...
                                                 <td>
                                                    <a title="{{__('admin.drivers.edit_drivers')}}" href="{{route($prefix.'_edit_drivers',['id' => $driver->id])}}"><i class="icon-xl la la-edit"></i></a>
                                                        <a title="{{__('admin.drivers.remove_drivers')}}" href="#" class="remove_btn" data="{{route($prefix.'_drivers_delete',['id' => $driver->id])}}"><i class="icon-xl la la-trash"></i></a>
                                                    </td>
                                                </tr>
                                                @endforeach
